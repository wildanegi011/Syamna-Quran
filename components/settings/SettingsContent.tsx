  ...
  const AccordionHeader = ({ id, icon: Icon, title, subtitle }: { id: typeof expandedSection, icon: any, title: string, subtitle?: string }) => (
    ...
    >
        <div className="flex items-center gap-4 text-left overflow-hidden">
            <div className={`p-2.5 rounded-xl transition-colors ${expandedSection === id ? 'bg-primary/20 text-primary' : 'bg-white/5 text-white/40 group-hover:bg-white/10'}`}>
                <Icon className="w-5 h-5" />
            </div>
            <div className="flex flex-col overflow-hidden">
                <span className={`text-sm font-bold transition-colors ${expandedSection === id ? 'text-white' : 'text-white/60'}`}>{title}</span>
                {subtitle && <span className="text-[10px] text-white/30 truncate">{subtitle}</span>}
            </div>
        </div>
        <ChevronDown className={`w-4 h-4 text-white/20 transition-transform duration-300 ${expandedSection === id ? 'rotate-180' : ''}`} />
    </button>
  );
                           ...
                           {/* UKURAN TEKS */}
                            <div className="space-y-6 bg-white/[0.02] p-4 rounded-2xl border border-white/5">
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center text-[11px]">
                                        <span className="text-white/40 uppercase font-bold tracking-widest">Huruf Arab</span>
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center text-[11px]">
                                        <span className="text-white/40 uppercase font-bold tracking-widest">Terjemahan</span>
                            ...
                            <div className="space-y-1">
                                {[
                                    ...
                                    <div key={i} className="flex items-center justify-between p-4 rounded-2xl hover:bg-white/[0.02] transition-colors">
                        ...
                        <div className="bg-white/[0.03] rounded-2xl overflow-hidden">
                            <div className="p-3 border-b border-white/5">
                                <div className="relative">
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/10" />
                                    <Input 
                                        placeholder="Cari Qori..." 
                                        value={searchReciter}
                                        onChange={(e) => setSearchReciter(e.target.value)}
                                        className="h-10 pl-10 bg-transparent border-none focus-visible:ring-0 text-sm placeholder:text-white/10"
                                    />
                                </div>
                            </div>
                            <div className="max-h-[300px] overflow-y-auto px-1.5 py-1.5 custom-scrollbar">
                                {filteredReciters?.map((r) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between p-3.5 px-4 rounded-xl text-left transition-all ${selectedReciterId === r.identifier ? 'bg-primary/10 text-primary font-bold' : 'text-white/40 hover:bg-white/5'}`}
                                    >
                            ...
                            <div className="max-h-[300px] overflow-y-auto px-1.5 py-1.5 custom-scrollbar">
                                {filteredTranslations?.map((t) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-4 p-3.5 px-4 rounded-xl text-left transition-all ${translationId === t.id ? 'bg-primary/10 text-primary font-bold' : 'text-white/40 hover:bg-white/5'}`}
                                    >
                                ...
                                {isTafsirLoading ? (
                                    ...
                                    filteredTafsirs.map((t) => (
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-4 p-3.5 px-4 rounded-xl text-left transition-all ${tafsirId === t.id ? 'bg-primary/10 text-primary font-bold' : 'text-white/40 hover:bg-white/5'}`}
                                        >
                                            <span className="text-xl shrink-0">{getLanguageMeta(t.languageName).flag}</span>

...
export const AyahRow = memo(function AyahRow({ ayah, surah, index, queue, tafsirText, onShowTafsir }: AyahRowProps) {
    ...
    return (
        <motion.div
            ...
            className={cn(
                "group flex flex-col gap-6 p-6 md:p-8 rounded-[2rem] transition-all duration-700 hover:bg-surface-container-highest/10 relative overflow-hidden border border-white/[0.02] scroll-mt-32 md:scroll-mt-40",
        ...
        >
            ...
            <div className="flex items-start justify-between gap-8 relative z-10 w-full">
                <div className="flex items-center gap-6 shrink-0">
                    <div className="relative w-14 h-14 flex items-center justify-center">
                        <div className={cn(
                            "w-12 h-12 rounded-2xl bg-surface-container-highest/20 flex flex-col items-center justify-center text-xs font-headline font-black transition-all duration-500",
                            ...
                            <span className={cn(ayah.surahInfo ? "text-[10px]" : "text-xs")}>{ayah.nomorAyat}</span>
                            {ayah.surahInfo && (
                                <span className="text-[6px] opacity-40 uppercase tracking-tighter leading-none mt-0.5">
                                    {ayah.surahInfo.nomor}
                                </span>
                            )}
                        </div>
                        ...
                        <div className={cn(
                            ...
                            <Button
                                ...
                                onClick={handlePlay}
                                className={cn(
                                    "w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 hover:scale-110 active:scale-95 p-0",
                ...
                <div
                    className={cn(
                        "flex-1 text-right font-arabic leading-[1.8] transition-all duration-1000",
            ...
            <div className="flex flex-col gap-4 relative z-10 pl-20">
                ...
                <div className="flex flex-col md:flex-row md:items-end gap-6 justify-between">
                    <p className={cn(
                        "text-sm md:text-lg font-body leading-relaxed transition-all duration-700 max-w-4xl",
            ...
            {/* Quick Actions (Hover) */}
            <div className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-3">
                <Tooltip>
                    <TooltipTrigger asChild>
                        <Button
                            variant="ghost"
                            size="icon"
                            className="p-2 rounded-full hover:bg-white/10 transition-colors text-muted-foreground hover:text-white h-11 w-11 cursor-pointer"
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <Button
                            variant="ghost"
                            size="icon"
                            className={cn("p-2 rounded-full hover:bg-white/10 transition-colors h-11 w-11 cursor-pointer", isCurrent ? "text-primary" : "text-muted-foreground")}
                            onClick={(e) => {
                                e.stopPropagation();

        ...
        <div className="flex-1 flex flex-col min-h-full pb-32">
            ...
            <main className="max-w-7xl mx-auto w-full px-6 md:px-12 -mt-10 z-20 space-y-12">
                ...
                <motion.div
                    ...
                    className="bg-foreground/[0.03] backdrop-blur-3xl border border-foreground/5 rounded-[1.25rem] p-6 md:p-8 shadow-2xl hover:shadow-primary/5 transition-all cursor-pointer relative overflow-hidden group/intro"
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10 relative z-10">
                        <div className="flex items-center gap-6 md:gap-8 w-full md:w-auto">
                            ...
                            <div className="flex-1 text-left">
                                <h3 className="text-xl md:text-2xl font-bold text-foreground mb-1 md:mb-1.5 tracking-tight group-hover/intro:text-primary transition-colors">Perkenalan Hijaiyah</h3>
                                <p className="text-foreground/40 text-[11px] md:text-sm italic font-body">Pelajari dasar-dasar makhorijul huruf sebelum mulai belajar Iqro.</p>
                            </div>
                        </div>
                        <Button
                            variant="default"
                            className="bg-foreground/10 hover:bg-primary hover:text-black text-foreground border border-foreground/10 rounded-full px-8 md:px-10 h-12 md:h-14 text-sm md:text-base font-bold tracking-tight transition-all"
                        >
                            Mulai Belajar
                        </Button>
                    </div>
                ...
                <div className="space-y-8">
                    <div className="flex items-center gap-4 px-2">
                        ...
                        <h3 className="text-[10px] md:text-xs font-headline font-black uppercase tracking-[0.4em] text-foreground/40">Level Pembelajaran</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {isLoading ? (
                            Array.from({ length: 6 }).map((_, idx) => (
                                <Skeleton key={idx} className="h-32 md:h-40 rounded-[2.5rem] bg-foreground/5" />
                            ))
                        ) : (
                            levels.map((level, idx) => (
                                <motion.div
                                    ...
                                    className={cn(
                                        "group relative flex flex-col p-6 h-[170px] transition-all duration-500 overflow-hidden border bg-foreground/[0.03] backdrop-blur-3xl rounded-[1.25rem] cursor-pointer",
                                        level.is_disabled
                                            ? "opacity-40 grayscale border-foreground/5"
                                            : "border-foreground/5 hover:border-foreground/10 hover:shadow-[0_15px_35px_-12px_rgba(0,0,0,0.15)] hover:-translate-y-1"
                                    )}
                                ...
                                >
                                    <div className="relative z-10 flex flex-col justify-between w-full h-full">
                                        <div className="flex justify-between items-start w-full">
                                            <div className="flex items-center gap-4 min-w-0">
                                                ...
                                                <div className={cn(
                                                    ...
                                                    level.is_disabled ? "bg-foreground/10 text-foreground/20" : level.color
                                                )}
                                                ...
                                                <div className="flex flex-col min-w-0">
                                                    <div className="flex items-center gap-2">
                                                        <h3 className={cn(
                                                            ...
                                                            !level.is_disabled ? "text-foreground group-hover:text-primary" : "text-foreground/40"
                                                        )}>{level.title}</h3>
                                                        {level.is_disabled && (
                                                            <span className="text-[8px] font-bold uppercase tracking-widest bg-foreground/5 text-foreground/20 px-2 py-0.5 rounded-full">Soon</span>
                                                        )}
                                                    </div>
                                                    <p className="text-foreground/40 text-xs italic truncate mt-0.5">{level.description}</p>
                                                </div>
                                            </div>

                                            <div className={cn(
                                                "w-8 h-8 rounded-full bg-foreground/5 flex items-center justify-center text-foreground/20 transition-all duration-300",
                                        ...
                                        <div className="flex items-center justify-between mt-auto">
                                            <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-foreground/20">Tahap {level.level_number}</span>
                                            {!level.is_disabled && (
                                    ...
                                    <div className={cn(
                                        ...
                                        level.is_disabled ? "bg-foreground/5" : "bg-primary/5 group-hover:bg-primary/10"
                                    )} />
                                </motion.div>
                            ))
                        )}
